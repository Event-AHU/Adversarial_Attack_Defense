...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, x, event_z, event_x,
        ...
        event_z = self.patch_embed(event_z)     # [:,:,:,:1000]
        event_x = self.patch_embed(event_x)     # B 768 1024
        ...
        if mask_z is not None and mask_x is not None: #pass
        ...
        if self.add_cls_token:#pass
            cls_tokens = self.cls_token.expand(B, -1, -1)
            cls_tokens = cls_tokens + self.cls_pos_embed

        if self.add_sep_seg:#pass
            x += self.search_segment_pos_embed
            z += self.template_segment_pos_embed
        #改 [1, 640, 768] Z[1, 64, 768] EZ[1, 64, 768]  X[1, 256, 768] EX[1, 256, 768]
        x = combine_tokens(z, event_z, x, event_x, mode=self.cat_mode)        # 64+64+256+256=640
        # x = torch.cat((event_z, event_x), dim=1)
        ...
        # 取出模版和搜索区域
        z = x[:, :lens_z_new*2] #模版 64
        x = x[:, lens_z_new*2:] # 搜索 256
        ...
        # x = x[:, :lens_x]   # RGB head 前256 rgb
        # x = torch.cat([event_x, x], dim=1) #为什么even_x不取融合后的256？？？？
