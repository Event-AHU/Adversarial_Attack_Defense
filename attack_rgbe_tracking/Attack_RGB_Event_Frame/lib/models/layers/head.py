...
class CenterPredictor(nn.Module, ):
    ...
    def cal_bbox(self, score_map_ctr, size_map, offset_map, return_score=False):
        ...
        idx_x = idx % self.feat_sz #计算拍平之前的xy值
        #ankor最大得到的idx
        idx = idx.unsqueeze(1).expand(idx.shape[0], 2, 1) #复制+reshape 中心点
        size = size_map.flatten(2).gather(dim=2, index=idx) #特征图拍平 取idx对应的wh
        offset = offset_map.flatten(2).gather(dim=2, index=idx).squeeze(-1) #偏移值
    ...
    def get_score_map(self, x):
        ...
        # ctr branch
        x_ctr1 = self.conv1_ctr(x) # 768*2 -> 256
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        if cfg.MODEL.HEAD.TYPE == "CORNER":
            ...
            raise ValueError()
        return corner_head
    elif cfg.MODEL.HEAD.TYPE == "CENTER": #走着
