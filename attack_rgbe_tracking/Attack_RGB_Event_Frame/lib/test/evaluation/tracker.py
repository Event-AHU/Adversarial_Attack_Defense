...
from event_CEUTrack.lib.test.evaluation.environment import env_settings
import time
import cv2 as cv

from event_CEUTrack.lib.utils.lmdb_utils import decode_img
...
class Tracker:
    ...
    def create_tracker(self, params,noise=None):
        tracker = self.tracker_class(params, self.dataset_name,noise=noise)
        return tracker

    def run_sequence(self, seq, debug=None,noise=None,attack="rgb"):
        ...
        if attack == "rgb" and self.initialize_noise is not None :
        ...
        # 初始化tracker
        tracker = self.create_tracker(params,noise=noise) #初始化ceutrack
        # 处理数据 并运行
        output,noise_test = self._track_sequence(tracker, seq, init_info,noise=noise,attack=attack)
        self.initialize_noise = noise_test


        return output,self.initialize_noise

    def _track_sequence(self, tracker, seq, init_info,noise=None,attack="rgb"):
        # Define outputs
        ...
        while not right_template:
            rgb_image = self._read_image(seq.frames[i])
        ...
        out = tracker.initialize(rgb_image, event_image, init_info, idx=i)
        if out is None:
            out = {}
        ...
        # for frame_num, (frame_path, frame_event_path) in enumerate(zip(seq.event_img_list[i+1:], seq.event_frames[i+1:]), start=1):
        for frame_num, (frame_path, event_img_path) in enumerate(zip(seq.frames[i+1:],  seq.event_img_list[i+1:]), start=1):
            ...
            if frame_num == 0 or frame_num == 1:
                ...
                att_y = 0
                att_t = 0
                cross_perturbation = 0
                print("Frame number:", frame_num)
                # print("Tracker noise:", tracker.noise)
                            
            # if frame_num % 1 == 1:
            #     att_per = 0 
            #     print('frame_num:', frame_num)
            out,noise = tracker.track(rgb_image, event_img,info=info,
                                        att_per=att_per,
