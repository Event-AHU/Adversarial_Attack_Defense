...
class Fe108(BaseVideoDataset):
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        elif os.path.exists(os.path.join(seq_path, 'frame{:04}.jpg'.format(frame_id))):
            return os.path.join(seq_path, 'frame{:04}.jpg'.format(frame_id))  
        else:
    ...
    def _get_event_sequence_path(self, seq_id):        ## get evemts' frames
        base_path = os.path.join(self.root, self.sequence_list[seq_id])
        voxel_folder = next(
            f for f in os.listdir(base_path) 
            if os.path.isdir(os.path.join(base_path, f)) 
            and f.endswith("voxel")
        )
        # 拼接完整路径
        full_path = os.path.join(base_path, voxel_folder)
        return full_path
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        frame_event_img_list = [self._get_frame(seq_event_path, f_id) for f_id in frame_ids] #图片
        if anno is None:
