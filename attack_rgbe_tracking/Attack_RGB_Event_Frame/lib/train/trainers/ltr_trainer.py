...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            if self.move_data_to_gpu:
                # 假设 self.device 是目标设备（如 "cuda" 或 "cpu"）
                data = {
                    k: v.to(self.device) if hasattr(v, 'to') else v  # 仅对张量调用 .to()
            ...
            # update statistics
            batch_size = data['template_images'].shape[loader.stack_dim] #取维度
    ...
    def _update_stats(self, new_stats: OrderedDict, batch_size, loader):
        ...
        if loader.training:
            ...
            for i, lr in enumerate(lr_list): #更新不同学习率下的参数
    ...
    def _print_stats(self, i, loader, batch_size):
        ...
        batch_fps = batch_size / (current_time - self.prev_time) #处理一个bs的时间
        average_fps = self.num_frames / (current_time - self.start_time) #处理一帧的时间
        prev_frame_time_backup = self.prev_time #备份上一次调用的时间戳
        self.prev_time = current_time #更新上次调用的时间戳为当前时间
        #累加数据读取时间
        ...
        if i % self.settings.print_interval == 0 or i == loader.__len__(): #只打印 间隔bs和最后一个bs的信息
