...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        for s,r in zip(['template', 'search'],['template_event', 'search_event']) :
            ...
            crops_s,boxes_s, att_mask_s, mask_crops_s, crop_coor_s = prutils.jittered_center_crop(data[s + '_images'],                                                                                     
                                                                                                jittered_anno,
                                                                                                data[s + '_anno'],
                                                                                                self.search_area_factor[s],
                                                                                                self.output_sz[s],
                                                                                                masks=data[s + '_masks'])
            crops_r,boxes_r, att_mask_r, mask_crops_r, crop_coor_r = prutils.jittered_center_crop(data[r + '_images'],                                                                                     
                                                                                                jittered_anno,
                                                                                                data[s + '_anno'],
                                                                                                self.search_area_factor[s],
                                                                                                self.output_sz[s],
            ...
            # data[s + '_event'] = torch.from_numpy(data[s + '_event'][0])  ## 数组转换成张量
            ...
            # x1, x2 = crop_coor_s[0][0] / 10, crop_coor_s[0][1] / 10
            # y1, y2 = crop_coor_s[0][2] / 10, crop_coor_s[0][3] / 10
