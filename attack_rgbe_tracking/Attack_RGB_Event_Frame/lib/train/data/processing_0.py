...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            # event frame
            data['template_event_images'], data['template_anno'], data['template_masks'] = self.transform['joint'](
                image=data['template_event_images'], bbox=data['template_anno'], mask=data['template_masks'])
            data['search_event_images'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_event_images'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)
        ...
        for s in ['template', 'search']:
            ...
            if event == 'frame':
                ...
                crops, boxes, att_mask, mask_crops, crop_coor = prutils.jittered_center_crop(data[s + '_event'],
                                                                                         jittered_anno,
                                                                                         data[s + '_anno'],
                                                                                         self.search_area_factor[s],
                                                                                         self.output_sz[s],
                                                                                         masks=data[s + '_masks'])
                # Apply transforms  [0.3427, 0.1020, 0.3068, 0.7882]
                data[s + '_event'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](
            ...
            crops, boxes, att_mask, mask_crops, crop_coor = prutils.jittered_center_crop(data[s + '_images'],
                                                                                         ...
                                                                                         self.output_sz[s],
                                                                                         masks=data[s + '_masks'])
            # Apply transforms  [-0.7379, -2.1766,  0.0075,  0.0192]
