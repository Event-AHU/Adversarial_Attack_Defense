...
class FrameGenerator(nn.Module):
    ...
    def hard_split_events_by_number(
        ...
        time_window = event_values.shape[0] // self.frame_number #16个事件点为一组
        frames = []
        for i in range(self.frame_number):
            start = i * time_window
            end = start + time_window
            ...
            pos_group_values = F.relu(group_values) #-1是正样本
            neg_group_values = F.relu(-group_values) #1是负样本
            ...
            frames.append(frame) #[c h w bs]

        frames = torch.stack(frames, dim=0)
        frames = frames.to_dense().permute(-1, 0, 1, 3, 2)
        return frames # torch.Size([1, 16, 2, 128, 128])
