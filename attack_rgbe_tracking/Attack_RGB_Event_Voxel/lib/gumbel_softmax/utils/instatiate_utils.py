    ...
    **kwargs,
    ...
    if "cifar" in name:
        ...
        train_data, test_data, val_data = random_split(
            data, [8000, 1000, 1000], torch.Generator().manual_seed(seed)
        )  # typ
        ...
        train_data, val_data = random_split(
            data, [1058, 118], torch.Generator().manual_seed(seed)
        )
    else:
...
#     #imgs_padded = pad_sequence(imgs, batch_first=True)

#     return (imgs.transpose(0, 1), labels)
# def _collate_fn(batch):
#     # 使用 default_collate 处理 batch
#     batch = default_collate(batch)
    
#     # 分离特征和标签
#     imgs, labels = batch
    
#     # 确保 imgs 和 labels 都是张量
...
def get_event_generator(cfg: dict):
    ...
    """
    name = cfg["attack"]["name"]  # torch
    generator_arguments = {
        "tau": cfg["attack"]["max_tau"], #2
        "sample_num": cfg["attack"]["sample_num"],#1 
        "use_soft_event": cfg["use_soft_event"], #False
    }
