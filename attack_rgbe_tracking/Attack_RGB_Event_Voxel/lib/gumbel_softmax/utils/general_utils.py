...
def get_target_label(cfg: dict, true_labels, device: torch.device):
    ...
    """
    if not cfg["targeted"]: #targeted=true
        target_labels = torch.tensor(true_labels, dtype=torch.int64, device=device)
    else: # 走这段 targeted=true
        _target_labels = []
        for true_label in true_labels:
            target_label = torch.tensor(
                get_random_target(cfg["dataset"]["num_class"], true_label), #生成与正确帧不同的类别
                dtype=torch.int64,
                device=device,
            )
...
def remove_misclassification(cfg: dict, test_data, model, frame_processor, device):
    ...
    for i, img_index in enumerate(index_of_attack_list): #一个序列 根据frame来分组
        # 取出对应的event数据和label 
        event, true_label = test_data[img_index]
        # 将event转为torch.Tensor 返回的是indices和values
        ...
        if not has_correct_prediction(cfg, model, unattack_frame, true_label):
            ...
            continue # 预测的和原始的标签不是一样继续
        # 原始的event数据 只存模型预测成功的信息
    ...
    # label数量
    correct_num = len(true_labels) #要攻击的类别
    print(f"model accuracy: {correct_num/cfg['num_pic']*100:.2f}%")
    return (
        correct_num, # label数量
        correct_events, # 原始的event数据
        true_labels,  # 原始的label
        true_values, # 张量的V
        true_indices,# 张量的I
        frame_list,# 帧列表
    )
