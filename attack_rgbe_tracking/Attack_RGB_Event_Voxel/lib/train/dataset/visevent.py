...
class VisEvent(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(self.root, 'voxel_train.txt')
            elif split == 'val':
        ...
        file_path = "/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c01/DATA/dataset/VisEvent/train/start_voxel_idx.txt"
        # 读取文件内容并转换为整数列表
        with open(file_path, "r") as file:
            self.start_frame_idx_list = [int(line.strip()) for line in file]

    def get_name(self):
        return 'visevent'

    def _get_sequence_list(self):
        with open(os.path.join(self.root, 'voxel_list.txt')) as f:
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        if seq_id < len(self.start_frame_idx_list):
            staart = self.start_frame_idx_list[seq_id]
        frame_ids = [x + staart for x in frame_ids]
        ...
        frame_event_img_list = self._get_event_frame(seq_event_path, frame_ids)
        ...
        seq_event_path = self._get_event_img_sequence_path(seq_id)
        frame_event_list = [self._get_frame(seq_event_path, f_id) for f_id in frame_ids]
        # frame_event_img_list是voxel frame_event_list是帧
