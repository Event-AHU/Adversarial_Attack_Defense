...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self.transforms = transforms #lib.train.data.transforms.ToGrayscale object at 0x7
    ...
    def __call__(self, **inputs):
        var_names = [k for k in inputs.keys() if k in self._valid_inputs] #取input中有的 self也有的
        for v in inputs.keys():
            if v not in self._valid_all: #检查input是否在valid_all中 
        ...
        if not joint_mode: # 跳过  如果是 False，则执行后续的代码块 
            out = zip(*[self(**inp) for inp in self._split_inputs(inputs)])
            return tuple(list(o) for o in out)
        #创建了一个新的字典 out，该字典只包含 inputs 中那些键存在于 self._valid_inputs 列表中的键值对
        out = {k: v for k, v in inputs.items() if k in self._valid_inputs}
        #out = {k: v || for k, v in inputs.items()|| if k in self._valid_inputs}
        for t in self.transforms: #应用一系列变换（transforms）到输入数据 out 上
            out = t(**out, joint=joint_mode, new_roll=new_roll) #bbox直接返回
        if len(var_names) == 1: #跳过
...
class TransformBase:
    ...
    def __call__(self, **inputs): ####数据变换Transform
        # Split input
        input_vars = {k: v for k, v in inputs.items() if k in self._valid_inputs} #输入数据
        input_args = {k: v for k, v in inputs.items() if k in self._valid_args}

        # Roll random parameters for the transform
        if input_args.get('new_roll', True):  #跳过
        ...
        outputs = dict()
        for var_name, var in input_vars.items():#[im bbox mask ]
            if var is not None:
                ...
                if var_name in ['coords', 'bbox']:
                    ...
                    params = self._rand_params #模版 true 搜索[Fasle]
    ...
    def _get_image_size(self, inputs): #获取图像和掩膜尺寸[260 346]
        im = None
    ...
    def transform_bbox(self, bbox, image_shape, *rand_params): #bbox变化 [260 346] true 
        """Assumes [x, y, w, h]"""
        # Check if not overloaded #没有被重载，则直接返回原始的 bbox
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:#根据参数 do_grayscale 决定是否将输入图像转换为灰度图像
