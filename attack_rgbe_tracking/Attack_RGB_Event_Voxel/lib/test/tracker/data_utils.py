...
class Preprocessor(object):
    ...
    def process(self, img_arr: np.ndarray, amask_arr: np.ndarray):
        ...
        img_tensor_norm = ((img_tensor / 255.0) - self.mean) / self.std  #  [-2.1179 2.6400] (1,3,H,W) [-2.1179 2.6400]
        # Deal with the attention mask
        amask_tensor = torch.from_numpy(amask_arr).to(torch.bool).cuda().unsqueeze(dim=0)  # (1,H,W)[1 128 128] 
