...
def generate_mask_cond(cfg, bs, device, gt_bbox):
    ...
    template_feat_size = template_size // stride #8 特征图尺寸
    #锁定目标区域
    if cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'ALL': #关注整合特征图
        box_mask_z = None
    elif cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'CTR_POINT':
        if template_feat_size == 8: #根据不同的特征图尺寸，选择不同的锁定区域
            index = slice(3, 4)
        ...
        raise NotImplementedError
            # [1 64]
    return box_mask_z #生成特征图关注区域 其余区域参与特征交互但不参与loss
