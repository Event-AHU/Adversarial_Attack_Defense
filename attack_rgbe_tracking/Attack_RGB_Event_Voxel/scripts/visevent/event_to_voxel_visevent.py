...
def transform_points_to_voxels(data_dict={}, voxel_generator=None, device=torch.device("cuda:0")):
    ...
    if num_points.shape[0] < save_voxel:
        features = voxels[:, :, 3] #一共4个维度 012是xyt 最后一个是p
        coor = coordinates[:, :] # 坐标
    else:
...
if __name__ == '__main__':
    ...
    data_path = "/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c01/DATA/dataset/VisEvent/train" #源文件路径
    save_path = "/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c01/DATA/dataset/VisEvent/train"
    match_file = "/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c01/DATA/dataset/VisEvent/train/trainpair1.txt"
    print(match_file)
    with open(match_file, 'r') as f:
        video_files = f.read().splitlines()  # 按行分割为列表
    # with open(match_file, 'r') as f:
    ...
    for videoID in range(len(video_files)):
        ...
        for FileID in tqdm(range(len(fileLIST))):
            ...
            for frame_no in trange(0, int(frame_num / dvs_img_interval) - 1):
                ...
                sub_event = events[start_idx:end_idx] #    # 截取属于当前帧的事件
                idx_length = end_idx - start_idx

                t = t_all[start_idx: end_idx]
                if start_idx == end_idx: # 无事件时的处理（避免除以零）
                    time_length = 0
                    ...
                    t = ((t-t[0]).float() / time_length) * 1000  # 归一化到 0~1000
